...
$reservations = $db->getAll("SELECT * FROM `doc_reservations` WHERE transactionid != '' AND ( CONCAT(`departure_date`,' ',STR_TO_DATE(`actual_pick_up_time`,  '%l:%i %p' )) BETWEEN NOW()+INTERVAL 19 hour AND NOW()+INTERVAL 1170 MINUTE ) ORDER BY `id` DESC");
echo "<pre>";
...
if ( count( $reservations ) > 0 ) {
    foreach( $reservations as $res ) {
        if (strpos($res["transactionid"], "txn_") !== false) {
            ...
            $html1 = '<p>Dear ' . $res["name"] . ' ' . $res["lastname"] . ', <br><br>Your reservation time is ' . $date . '. It will be in next 24 hours.<br><br>Thanks</p>';
